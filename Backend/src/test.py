...
def PrimaryAgent(state: AgentState):
    ...
    input = ''
    print("\nstate Primary Agent ", state)    
    if state.is_fulfullied == True:
...
def RAGAgentToQueryToHumanResponse(state: AgentState):
    ...
    system_prompt = (
        """You are a human response generator. You will get Two documents, your job is to convert that document to response of input.
            Don't add any info other than provided document. Response should be according to input.
            Query result: {docs}
            User input: {input}
        """)
...
def GenerateHumanResponse(state: AgentState):
    ...
    if state.output != None:
        state.output = state.output + " " + response
    else:
...
def OutputChecker(state: AgentState):
    system_prompt = (
    """You are an Output Checker responsible for validating if the output fully addresses the input query. 
    Your role is to analyze the output, ensuring all aspects explicitly mentioned in the input are addressed.
    Your goal is to check if output aligns with input.
    
    - If the output completely answers the query, return `fulfilled: True`.
    - If any part of the query is not answered or is unclear, return `fulfilled: False` and specify what is missing in `missing_info`.
    - Do **not** include missing details or aspects not explicitly mentioned in the input query.
    
    Be concise, focus strictly on the input query, and avoid over-interpreting or adding unnecessary missing details which input has not.
...
def RAGAgent(state: AgentState):
    ...
    state.database = relevance.database
    print("\nresponse SQLAgent: ", state.database)
    return state


def SearchToolAgent(state: AgentState):
    pass
...
def InputRefiner(state: AgentState):
    ...
    print("\nresponse InputRefiner: ", response)
    return state
...
workflow.add_edge("InputRefiner", END)

# workflow.add_edge(START, "PrimaryAgent")
...
# result = graph.invoke({"input": "tell me about the flight from uk or to uk and its prices as well, and i also want the details of hotel in usa. How can I contact GlobeFLights?"})
